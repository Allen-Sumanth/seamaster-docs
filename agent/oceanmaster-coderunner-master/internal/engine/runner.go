...
type MatchResults struct {
	MatchID    int      `json:"match_id"`    // ID of the match
	Winner     int      `json:"winner"`      // Winner ID if someone won, 2 if draw or early exit
	...
	LogURL     string   `json:"log_url"`     // URL to the log file populated at the end
}
		...
		*out = PlayerMoves{}
		...
		if err != nil {
			...
			if errors.Is(err, io.EOF) {
				return fmt.Errorf("python process terminated unexpectedly (EOF)")
			}
