...
type GameManager struct {
	cfg      *config.Config
	matches  map[int]*engine.Match
	mu       sync.Mutex
	s3Client *minio.Client
}

func NewGameManager(cfg *config.Config, s3Client *minio.Client) *GameManager {
	return &GameManager{
		cfg:      cfg,
		matches:  make(map[int]*engine.Match),
		mu:       sync.Mutex{},
		s3Client: s3Client,
	}
}
...
func (gm *GameManager) NewMatch(job MatchJob) error {
	matchDir := path.Join(gm.cfg.HostSubmissionPath, fmt.Sprint(job.ID))
	p1Dir := path.Join(matchDir, "p1")
	p2Dir := path.Join(matchDir, "p2")
	logFile := path.Join(matchDir, "log.txt")
	...
	gl.Log(engine.GameLogDebug, fmt.Sprintf("Match ID %d at %s", job.ID, time.Now().Format(time.RFC3339)))
	...
	if err := PostResults(gm.s3Client, gm.cfg, m.ID, results, logFile); err != nil {
		err = fmt.Errorf("post results: %w", err)
		gl.Log(engine.GameLogError, "Failed to post results")
		_ = os.Rename(logFile, path.Join(matchDir, fmt.Sprintf("log-%d.txt", time.Now().UnixNano()))) // move log to a file with timestamp to avoid overwriting in case of retry
		return err
	} else {
		_ = os.Remove(matchDir) // fine if it fails
	}
